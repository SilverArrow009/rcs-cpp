...
run_simulation() {
    ...
    if [ -f "$GEM5_EXE" ]; then
        "$GEM5_EXE" "$config" "./$build_dir/rcs_benchmark" --bin-args "$args"
    else
...
# Run Baseline
# run_simulation "baseline" "NO_USE_CMPLX_EXT" "$BASELINE_CONFIG" "4 5"

# Run Custom
run_simulation "custom" "USE_CMPLX_EXT" "$CUSTOM_CONFIG" "4 5"
